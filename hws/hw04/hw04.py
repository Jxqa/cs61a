...
def replace_leaf(t, find_value, replace_value):
    ...
    elif is_leaf(t) and label(t) != find_value:
        return tree(label(t))
    else:
...
def has_path(t, phrase):
    ...
    "*** YOUR CODE HERE ***"
    if label(t) == phrase[0]:
        if label(t) == phrase[len(phrase) - 1]:
            return True
        for b in branches(t):
            has_path(b, phrase[1:])
        return True
...
def div_interval(x, y):
    ...
    reciprocal of y."""
    "*** YOUR CODE HERE ***"
    assert upper_bound(y) * lower_bound(y) != 0 and 1/upper_bound(y) < 1/lower_bound(y)
...
def quadratic(x, a, b, c):
    ...
    """
    "*** YOUR CODE HERE ***"
    c1 = a *(lower_bound(x) ** 2) + b * lower_bound(x) + c
    c2 = a *(upper_bound(x) ** 2) + b * upper_bound(x) + c
    ep = -b/(2*a)
    c3 = a *(ep ** 2) + b * ep + c
    if a == 0 or not lower_bound(x) <= ep <= upper_bound(x):
        return interval(min(c1, c2), max(c1, c2))
    else:
        if a > 0:
            return interval(c3, max(c1, c2))
        elif a < 0:
            return interval(min(c1, c2), c3)
