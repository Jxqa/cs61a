...
class VendingMachine:
    ...
    def vend(self):
        if self.stock == 0:
            ...
            k = self.price - self.balance
            return 'You must add ${0} more funds.'.format(k)
        elif self.balance == self.price:
            self.stock -= 1
            return 'Here is your {0}.'.format(self.product)
        elif self.balance > self.price:
            m = self.balance - self.price
            self.balance = 0
            self.stock -= 1
            return 'Here is your {0} and ${1} change.'.format(self.product, m)
...
class Coin:
    ...
    def worth(self):
        "*** YOUR CODE HERE ***"
        age = Mint.current_year - self.year - 50
        return self.cents + (age if age > 0 else 0)
