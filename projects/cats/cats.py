...
def accuracy(typed, reference):
    ...
    if len(typed_words) == 0:
        return 0.0
    while i <= min(len(typed_words) - 1, len(reference_words) - 1):
...
def autocorrect(user_word, valid_words, diff_function, limit):
    ...
    if user_word in valid_words:
        ...
        for str in valid_words:
            if diff_function(user_word, str, limit) < key:
                key = diff_function(user_word, str, limit)
                result = str
        if key > limit:
            return user_word
        else:
...
def time_per_word(times_per_player, words):
    ...
    while m < len(times_per_player):
        lst = []
        while i < len(times_per_player[m]):
            lst += [times_per_player[m][i] - times_per_player[m][i - 1]]
            i += 1
        times += [lst]
        i = 1
        m += 1
...
def fastest_words(game):
    ...
    for w in words:
        w_time = 10000
        for p in players:
            if time(game, p, w) < w_time:
                w_time = time(game, p, w)
        for p in players:
            if time(game, p, w) == w_time:
                break
        lst[p] += [word_at(game, w)]
    return lst
    # END PROBLEM 10
